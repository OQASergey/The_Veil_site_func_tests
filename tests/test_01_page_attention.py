...
def test_attention_text():
    ...
    browser.element('[id="at2"]').should(have.text(
        'Данный сайт является небольшим справочным ресурсом для личного использования по настольной '
        'ролевой игре the Veil (авторы: Фрейзер и Кайл Симмонс). Сайт ничего не пропогандирует и не '
        'распространяет. На сайте находятся только игровые буклеты и демонстрационные материалы. '
        'Для ознакомления с полной версией книги обращайтесь на официальную страницу настолько ролевой игры'
        ' "The Veil".'))
